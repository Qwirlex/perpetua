...
**Added after the Task 6 review, do not skip.** Every lens ships the whole flattened source, so a
large verified contract multiplies token spend by six with nothing to stop it. Cap it here, in
`prepare_target`, not inside the lenses. Add a module constant `MAX_PROMPT_SOURCE_CHARS` defaulting
to 120000, and when the flattened source is longer, keep the head, cut the middle, and append a
plain marker line saying the source was truncated and by how many characters. Record the truncation
in the report coverage as a not checked entry, since a buyer must not be told the whole contract was
read when it was not. Test that a source under the cap passes through untouched, that a source over
it is cut, keeps its beginning, and carries the marker, and that the coverage records it.
...
def run_deep_audit(prepared: Prepared, *, llm=None, summarize=None, signing_key: str | None = None) -> Report:
    ...
    # Ids are stamped once, here, after the set is final. merge_findings deliberately
    # does not assign them, since it runs twice per audit and ids must not shift.
